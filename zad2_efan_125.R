...
#Отфильтруем данные за летний период с 1 июны (151 день) по 31 августа (242 день)
...
# 4 МОДЕЛЬ,добавим в неё переменные, полученные при помощии функции anova() с пометками "***" 
...
qplot(un_h2o_flux, h2o_flux, data = testing_eddypro_unq) + geom_line(aes(y = predict(model3, testing_eddypro_unq)))
